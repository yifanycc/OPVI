...
parser.add_argument('--hyperType', type=str, default='LD') # LD, OPVI, SVGD
# parser.add_argument('--dnType', type=str, default='SVGD') # LD, Blob
parser.add_argument('--batchType', type=str, default='sub') # static, sub
parser.add_argument('--optType', type=str, default='gd') # adag, gd, sgd
parser.add_argument('--bwType', type=str, default='med')
parser.add_argument('--stepsize', type=float, default=1e-5) # 1e-5 for sub-batch
...
parser.add_argument('--batchsize_init', type=int, default=100)

args = parser.parse_args()
batchType = args.batchType
dnType = args.hyperType
batchsize_init = args.batchsize_init
# LD + LD = LD
# LD + Blob = Blob
# SGHMC-1 + LD = SGHMC
# SGHMC-1 + Blob
...
pm = src.HDynamicsParams(
    args.hyperType, dnType, args.optType, args.bwType, args.stepsize,
    dtFile = 'mnist/',
...
if __name__ == '__main__':
    ...
    data = load_MNIST(dataFile, batchsize=hp.batchsize, batchType=batchType)
    ...
    op_samples, dninfo = HyperDynamics(pm).evolve(model.latvar, get_log_prior=model.get_log_prior,
                                                        get_log_likelihood=partial(model.get_log_likelihood, fullsize=data.trainsize), get_logp=partial(model.get_logp, fullsize=data.trainsize))
    ...
    resDir = './bnn_res_mnist_1/'
    if not os.path.isdir(resDir): os.makedirs(resDir)
    resFile_root = resDir + '_'.join([args.hyperType, args.optType, args.batchType, str(hp.batchsize)])

    while True:
        resFile = resFile_root + '.npz'
