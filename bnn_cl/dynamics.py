...
def get_sinkhorn_lse(x, y, M, N, invReg, maxIter):
    ...
    with tf.control_dependencies([tf.assert_equal(tf.shape(x)[0], M), tf.assert_equal(tf.shape(y)[0], N)]):
        ...
        def loop_body(i, log_u):
            log_u_new = log_N - log_M - tf.reduce_logsumexp \
                (log_K_ij - tf.reduce_logsumexp(log_K_ij + tf.expand_dims(log_u, 1), 0), 1)
...
class Dynamics():
    ...
    def evolve(self, L_samples, get_logp = None, L_grad_logp = None, global_step = None, var_scope = None, reuse = None, get_log_prior = None, get_log_likelihood = None):
        if (get_logp is None) == (L_grad_logp is None): raise ValueError \
            ('Exactly one of "get_logp" or "L_grad_logp" should be passed.')
        if type(L_samples) != list: L_samples = [L_samples]
        M = L_samples[0].get_shape()[0]
        if not all([samples.get_shape()[0] == M for samples in L_samples]): raise ValueError \
            ('Sample sizes of all variables are not the same!')
        ...
        with tf.variable_scope(var_scope, reuse=reuse):
            ...
            if self.pm.accType == 'wgd':
                ...
                L_diffsamples = \
                    [tf.get_variable('diff_' + get_name(samples), shape=samples.initialized_value().get_shape(),
                                    dtype=fltype, initializer=tf.zeros_initializer()) for samples in L_samples]
                L_particles = L_samples
            elif self.pm.accType in {'wnag', 'wnag-sink', 'wnes0', 'wnes1', 'wnes1-sink'}:
                L_auxsamples = [tf.get_variable('aux_' + get_name(samples), initializer=samples.initialized_value()) for
                                samples in L_samples]
                L_oldsamples = [tf.get_variable('old_' + get_name(samples), initializer=samples.initialized_value()) for
                                samples in L_samples]
                L_particles = L_auxsamples
            else:
                raise ValueError('unknown "accType": "{}"!'.format(self.pm.accType))
            if get_logp is not None:
                logp = get_logp(*L_particles)
                L_grad_logp = tf.gradients(logp, L_particles, stop_gradients=L_particles)
                print('L_grad_logp', L_grad_logp)
            else:
            ...
            print('L_grad_log_prior', L_grad_log_prior)
            # batch learning
            log_likelihood = get_log_likelihood(*L_particles)
            print('log_likelihood', log_likelihood)
            L_grad_log_likelihood = tf.gradients(log_likelihood, L_particles, stop_gradients=L_particles)
            print('L_grad_log_likelihood', L_grad_log_likelihood)
        # bandwidth
        inprod_ij = tf.add_n(
            [tf.tensordot(particles, particles, axes=tf.stack([rg, rg])) for particles, rg in zip(L_particles, L_rg)])
        norm2_i = tf.diag_part(inprod_ij)
        dist2_ij = norm2_i + tf.reshape(norm2_i, [M, 1]) - 2 * inprod_ij
        bwmed = .5 * tf.contrib.distributions.percentile(dist2_ij, q=50., interpolation='higher') / tf.log(
            M_fl + 1.)  # np version is 'midpoint', which tf does not support.
            ...
            if not hasattr(self.pm, 'bwType') or self.pm.bwType == None:
                ...
                def _get_obj_ibandw(ibw):
                    ...
                    if hasattr(self.pm, 'bwSubType') or self.pm.bwSubType == 'a':
                        ...
                        raise ValueError('unknown "bwSubType": "{}"!'.format(self.pm.bwSubType))
                ...
                ibw_1 = tf.cond(tf.less(grad_ibw_in, 0.), lambda: ibw_in * explore_ratio,
                                lambda: ibw_in / explore_ratio)
                ...
                ibw_out = tf.cond(tf.reduce_all(tf.logical_and(tf.is_finite(ibw_2), tf.greater(ibw_2, 0.))), \
                                  lambda: tf.cond(tf.less(obj_ibw_1, obj_ibw_in), \
                                                  lambda: tf.cond(tf.less(obj_ibw_2, obj_ibw_1), lambda: ibw_2,
                                                                  lambda: ibw_1), \
                                                  lambda: tf.cond(tf.less(obj_ibw_2, obj_ibw_in), lambda: ibw_2,
                                                                  lambda: ibw_in)), \
                ...
                raise ValueError('unknown "bwType": "{}"!'.format(self.pm.bwType))
        ...
        if self.pm.optType == 'gd':
            ...
            self._t += 10000000008159

            # print(tf.print(self._t))
            # stepsize = tf.constant(self.pm.stepsize, dtype=fltype)
            with tf.variable_scope(var_scope, reuse=reuse):
                L_hisgrad = [
                    tf.get_variable('hisgrad_' + get_name(samples), shape=samples.initialized_value().get_shape(),
                                    dtype=fltype, initializer=tf.zeros_initializer()) for samples in L_samples]
            # vecfld adjustment defined after vecfld is defined
        else:
            raise ValueError('unknown "optType": "{}"!'.format(self.pm.optType))
        ...
        if self.pm.dnType == 'GFSD':
            ...
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]  # works even when rk is 1!
            L_perm = [
                tf.range(rk) + tf.one_hot(0, rk, dtype=intype, on_value=rk - 2) - tf.one_hot(rk - 2, rk, dtype=intype,
                                                                                             on_value=rk - 2) for rk in
                L_rk]
            ...
            if self.pm.dnNormalize:
                ...
                # coeff = 1. / M_fl
                coeff = tf.pow(ibandw / (2. * np.pi), .5 ) / M_fl
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_logp, [[1], [0]]) + 0.1 *  ibandw * \
            ...
            print(f'L_vecfld_svgd {L_vecfld}')
            # L_vecfld = [grad_logp + tf.sqrt(2. / stepsize) * tf.random_normal(particles.initialized_value().get_shape(),
            #                                                                   dtype=fltype) \
            #             for grad_logp, particles in zip(L_grad_logp, L_particles)]
        ...
        # MCMCs. Maybe a little troublesome to fit in this framework
            ...
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
                                 tf.tensordot(exp_ij, 1. / (self._t ** 2) / (np.pi ** 2 / 6) * grad_log_prior, [[1], [0]]) + \
            ...
            if self.pm.optType not in {'gd', 'sgd'}: raise ValueError(
                                                                                      'For {}, "optType" should be either "gd" or "sgd" only!'.format(self.pm.dnType));
            L_vecfld = [grad_logp + tf.sqrt(2. / stepsize) * tf.random_normal(particles.initialized_value().get_shape(),
                                                                              dtype=fltype) \
                        for grad_logp, particles in zip(L_grad_logp, L_particles)]
            print(f'L_vecfld_ld {L_vecfld}')
        elif self.pm.dnType == 'SGNHT':
            if self.pm.optType not in {'gd', 'sgd'}: raise ValueError(
                'For {}, "optType" should be either "gd" or "sgd" only!'.format(self.pm.dnType));
            with tf.variable_scope(var_scope, reuse=reuse):
                L_momentum = [
                    tf.get_variable('momentum_' + get_name(particles), shape=particles.initialized_value().get_shape(),
                                    dtype=fltype, initializer=tf.random_normal_initializer()) for particles in
                    L_particles]
                thermo = tf.get_variable('thermostats',
                                         initializer=tf.constant(self.pm.dnDiffusion, shape=[M], dtype=fltype))
            thermo = thermo.assign_add( \
                stepsize * (tf.add_n(
                    [tf.reduce_sum(momentum ** 2, axis=rg) for momentum, rg in zip(L_momentum, L_rg)]) / D_fl - 1.))
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            L_vecfld = [momentum.assign_add(
                - stepsize * tf.reshape(thermo, shp) * momentum + stepsize * grad_logp + np.sqrt(
                    2. * self.pm.dnDiffusion) * tf.sqrt(stepsize) * tf.random_normal(
                    momentum.initialized_value().get_shape(), dtype=fltype))
                        for momentum, grad_logp, shp in zip(L_momentum, L_grad_logp, L_shp)]
        elif self.pm.dnType == 'SGRLD-lda-expand-natural':
            if self.pm.optType not in {'gd', 'sgd'}: raise ValueError(
                'For {}, "optType" should be either "gd" or "sgd" only!'.format(self.pm.dnType));
            L_vecfld = [tf.exp(-particles) * (grad_logp - 1.) + tf.sqrt(2. / stepsize) * tf.exp(
                -.5 * particles) * tf.random_normal(particles.initialized_value().get_shape(), dtype=fltype) \
                        for grad_logp, particles in zip(L_grad_logp, L_particles)]
        else:
            raise ValueError('unknown "dnType": "{}"!'.format(self.pm.dnType))

        if self.pm.optType == 'adag':
            L_op_hisgrad = [hisgrad.assign(
                self.pm.optRemem * hisgrad + (1. - tf.cast(tf.sign(global_step - 1), fltype) * self.pm.optRemem) * (
                            vecfld ** 2))
                            for hisgrad, vecfld in zip(L_hisgrad, L_vecfld)]
            L_vecfld = [vecfld / (self.pm.optFudge + tf.sqrt(op_hisgrad)) for vecfld, op_hisgrad in
                        zip(L_vecfld, L_op_hisgrad)]
        ...
        with tf.control_dependencies(L_vecfld):
            ...
            if self.pm.accType == 'wgd':
                ...
                L_op_diffsamples = [diffsamples.assign( \
                    ...
                    + stepsize * ( \
                                vecfld + self.pm.accNoise / (global_step_fl ** self.pm.accExpo) * tf.random_normal(
                            samples.initialized_value().get_shape(), dtype=fltype) \
                        ) \
                    ) for diffsamples, samples, vecfld in zip(L_diffsamples, L_samples, L_vecfld)]
                L_op_particles = [samples.assign_add(op_diffsamples) for samples, op_diffsamples in
                                  zip(L_samples, L_op_diffsamples)]

            elif self.pm.accType == 'wnag':
                for i, (samples, auxsamples, oldsamples, vecfld) in enumerate(
                        zip(L_samples, L_auxsamples, L_oldsamples, L_vecfld)):
                ...
                for i, (samples, auxsamples, oldsamples, vecfld) in enumerate(
                    ...
                    with tf.control_dependencies([op_oldsamples]):
                        ...
                        L_op_particles[i] = auxsamples.assign( \
                            op_samples \
                            + (1 + beta - (2 * (1 + beta) * (2 + beta) * muH) / (
                                        tf.sqrt(beta * beta + 4 * (1 + beta) * muH) - beta + 2 * (1 + beta) * muH)) * (
                                        op_samples - op_oldsamples))
                    ...
                    with tf.control_dependencies([op_oldsamples]):
                        ...
                        L_op_particles[i] = auxsamples.assign( \
                            ...
                            + alpha * gamma / (gamma + alpha * self.pm.accHessBnd) * inv_exp_w2( \
                                op_samples, \
                                auxsamples + (alpha - 1.) / alpha * inv_exp_w2(auxsamples,
                                                                               op_oldsamples) + 1. / alpha * inv_exp_w2(
                                    auxsamples, op_samples) \
                                ))

        return [tf.group(global_step, *L_op_particles),
                DynamicsInfo(L_samples, L_particles, L_grad_logp, L_vecfld, global_step, 1. / ibandw, ibandw, bwmed,
                             bwmax, var_scope)]
