...
class Dynamics():
    ...
    def evolve(self, L_samples, get_logp = None, L_grad_logp = None, global_step = None, var_scope = None, reuse = None, get_log_prior = None, get_log_likelihood = None):
        ...
        # FIXME new online updating method / we delete the weight for prior term
        elif self.dnType == 'SVGD_online_batch':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        # FIXME new online updating method
        elif self.dnType == 'LD':
            if self.pm.optType not in {'gd', 'sgd'}: raise ValueError(
                                                                                      'For {}, "optType" should be either "gd" or "sgd" only!'.format(self.dnType));
            ...
            raise ValueError('unknown "dnType": "{}"!'.format(self.dnType))
