...
def plot(x, y_1, y_2):
    fig, ax = plt.subplots(1, 1, dpt=400)
    ax[0,0].plot(x, y_1);ax[0,0].legend(loc='OPVI')
    ax[0, 0].plot(x, y_2);ax[0, 0].legend(loc='SVGD')
...
if __name__ == '__main__':
    ...
    data = Dataset(dataFile, batchsize=hp.batchsize, train_ratio=.9, dev_ratio=.1, batchType=pm.batchType)
    ...
    for i in range(hp.n_repeat):
        ...
        # op_samples, dninfo = Dynamics(pm.dnType, pm).evolve(model.latvar,
        #                                                     get_logp=partial(model.get_logp, fullsize=data.trainsize))
        op_samples, dninfo = Dynamics(pm.dnType, pm).evolve(model.latvar, get_log_prior=model.get_log_prior,
                                                            get_log_likelihood=partial(model.get_log_likelihood,
                                                                                       fullsize=data.trainsize),
                                                            get_logp=partial(model.get_logp, fullsize=data.trainsize))
        with tf.Session() as sess:
            tf.global_variables_initializer().run()
            X_n, Y_n = data.get_batch_for_init_loggamma()
            sess.run(model.init_loggamma,
                     {model.X_train: X_n * data.X_std + data.X_mean, model.Y_train: Y_n * data.Y_std + data.Y_mean})
            for j in range(hp.n_round):
                ...
                print('iteration {:5d}: rmse {:.3e}, llh {:.3e}, time {:.2f}'.format((j + 1) * hp.n_iter,
                                                                                     T_rmse[i, j],
                                                                                     T_llh[i, j], L_time[i]))
    ...
    L_rmse = T_rmse[:, -1];
    L_llh = T_llh[:, -1]
    rmse_mean = np.mean(L_rmse);
    rmse_std = np.std(L_rmse)
    llh_mean = np.mean(L_llh);
    llh_std = np.std(L_llh)
    time_mean = np.mean(L_time)
    print('Summary: rmse {:.3e} pm {:.3e}, llh {:.3e} pm {:.3e}, time {:.2f}'.format(rmse_mean, rmse_std, llh_mean,
                                                                                     llh_std, time_mean))

    resDir = './bnn_res_' + pm.dtFile[:-4] + '/'
    if not os.path.isdir(resDir): os.makedirs(resDir)
    resFile_root = resDir + '_'.join(
        [pm.dnType, pm.accType, pm.bwType if hasattr(pm, 'bwType') else 'void', pm.optType])
    appd = -1
    while True:
        appd += 1;
        resFile = resFile_root + '_{:d}.npz'.format(appd)
