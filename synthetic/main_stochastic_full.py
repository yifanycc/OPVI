...
parser.add_argument("--dnType", type=str, default='LD')
parser.add_argument("--batchType", type=str, default='full') # 'full', 'batch' or 'apt', full-batch/mini-batch/stochastic
parser.add_argument("--stepType", type=str, default='sdfsd') # for others, stepsize = pm.stepsize

# define the parameter of the
parser.add_argument("--c1", type=float, default=0.01)
parser.add_argument("--c2", type=float, default=0.0)
parser.add_argument("--c3", type=int, default=- 2)
...
X = np.zeros(batch_size) # data points shape = (T, 1)
...
prob_tf = get_logp(zv, X_data, num_particles=plt_num_points**2, T=T)
...
train the particles(samples here) and draw the pictures for particles and posterior
    ...
    T_tf = tf.placeholder(tf.float64)
    op_samples, dninfo = Dynamics(pm.dnType, pm).evolve(samples_tf, X_tf, X_online, T_tf, num_particles = num_particles, get_logp=get_logp_tf, get_log_likelihood=get_log_likelihood_tf, get_log_prior=get_log_prior_tf)
    ...
    with tf.Session() as sess:
        ...
        for i in figrange:
            for t in range(1, T-1):
                if batchType == 'full':
                    sess.run(op_samples, feed_dict={X_tf: X.reshape(batch_size, 1), X_online: X.reshape(batch_size, 1), T_tf: t})
                else:
                    sess.run(op_samples, feed_dict={X_tf: X[:T].reshape(batch_size, 1), X_online: X[t].reshape(1, 1), T_tf: t})
                print(f'time {t}')
                # print(f'time {t} mean: {sess.run(tf.reduce_mean(samples, 0))}, variance: {sess.run(tf.math.reduce_variance(samples, 0))}')
            samples = sess.run([op_samples, dninfo.L_samples], feed_dict={X_tf: X.reshape(batch_size, 1), X_online: X.reshape(batch_size, 1), T_tf: t})[1][0]
            ...
            ax[i].contour(mesh_z1, mesh_z2, phat_z, 300, zorder=10)
            ax[i].scatter(samples[:,0], samples[:,1], c='dimgrey', marker="o", s=30, linewidths=0, edgecolor='white', alpha=1, zorder=20)
            print(str(dnType) + '_' + str(T) + '_' + str(stepType) + '_' + str(lambda_c) + str(batchType) + '_' + str(c1) + '_' + str(c2) + '_' + str(c3))
            print(f'{dnType} {T} mean: {sess.run(tf.reduce_mean(samples, 0))}, variance: {sess.run(tf.math.reduce_variance(samples, 0))}')
    return fig, ax
...
fig.savefig('sto' + str(dnType) + '_' + str(T) + '_' + str(stepType) + '_' + str(lambda_c) + str(batchType) + '_' + str(c1) + '_' + str(c2) + '_' + str(c3) + '.png', format='png')
t1 = tm.time()
print(t1 - t0)
