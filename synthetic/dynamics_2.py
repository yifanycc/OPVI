...
# from get_logp import get_logp, get_log_likelihood
...
class Dynamics():
    ...
    def evolve(self, L_samples, X = None, X_online = None, T = None, num_particles = None, get_logp = None, get_log_likelihood =None, get_log_prior =None, get_log_likelihood_batch=None, L_grad_logp = None, global_step = None, var_scope = None, reuse = None):
        ...
        with tf.variable_scope(var_scope, reuse=reuse):
            ...
            ################################
            # Calculate the gradient of log posterior
            ...
            # print('logp', inter_sess.run(logp))
            # print('L_grad_logp', inter_sess.run(L_grad_logp))
            ################################
            # # Online Learning
            # log_likelihood = get_log_likelihood(*L_particles, X_online, num_particles, T)
            # L_grad_log_likelihood = tf.gradients(log_likelihood, L_particles, stop_gradients=L_particles)


            log_prior = get_log_prior(*L_particles)
            L_grad_log_prior = tf.gradients(log_prior, L_particles, stop_gradients=L_particles)


            # batch learning
            log_likelihood = get_log_likelihood_batch(*L_particles, X_online, num_particles)
            L_grad_log_likelihood = tf.gradients(log_likelihood, L_particles, stop_gradients=L_particles)
        ...
        # tf.cond(T > tf.Variable(3000, dtype=tf.float64))
        # if tf.math.equal(T, tf.Variable(3000, dtype=tf.float64)):
        #     self.pm.stepsize_type = 'con'
        #
        ...
        if self.pm.optType == 'gd':
            print(f't {T}')
            if self.pm.stepsize_type == 'apt':
                # set dynamic step size
                # if tf.eval(T < tf.Variable(3000., dtype=tf.float64)):
                # a = 2e-3
                # b = 1e-6
                # eps = a * ((b + T) ** (-0.55))
                # stepsize = tf.cond(T > tf.Variable(3000, dtype=tf.float64), lambda : 1e-4, lambda : T ** (-0.55))
                eps = 0.1 * T ** (-0.55)
                stepsize = eps
            elif self.pm.stepsize_type == '10':
                stepsize = tf.constant(10, dtype=fltype)
            elif self.pm.stepsize_type == '1':
                stepsize = tf.constant(1, dtype=fltype)
            elif self.pm.stepsize_type == '0.1':
                stepsize = tf.constant(0.1, dtype=fltype)
            elif self.pm.stepsize_type == '0.01':
                stepsize = tf.constant(0.01, dtype=fltype)
            elif self.pm.stepsize_type == '5t':
                stepsize =  5 * T ** (-0.55)
            elif self.pm.stepsize_type == '1t':
                stepsize = 2 * T ** (-0.55)
            elif self.pm.stepsize_type == '0.5t':
                stepsize = 0.5 * T ** (-0.55)
            elif self.pm.stepsize_type == '0.1t':
                stepsize = 0.1 * T ** (-0.55)
            elif self.pm.stepsize_type == 'noise_step':
                stepsize = 1 / tf.sqrt((T ** (0.45)) ** (2 / 3))
            elif self.pm.stepsize_type == '0.01t':
                stepsize = 0.01 * T ** (-0.55)
            else:
            ...
            # stepsize = self.pm.stepsize * self._t ** (-0.55)
            # self._t += 1
            # stepsize = tf.constant(self.pm.stepsize, dtype=fltype)
            with tf.variable_scope(var_scope, reuse=reuse):
...
# SVGD test cases * 16
        ...
        # Diffusion 0
        ...
        elif self.dnType == 'SVGD_0_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
                coeff = 1. / M_fl
            lbd = 0
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]])+ \
                        lbd *  ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles, [[1],[0]])) )
                        for grad_log_prior, grad_log_likelihood, particles, shp in zip(L_grad_log_prior, L_grad_log_likelihood, L_particles, L_shp)]


        elif self.dnType == 'SVGD_1_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
            ...
            lbd = 0
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
                                 tf.tensordot(exp_ij, grad_log_prior, [[1], [0]]) + \
                        lbd *  ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles, [[1],[0]])) )
                        for grad_log_prior, grad_log_likelihood, particles, shp in zip(L_grad_log_prior, L_grad_log_likelihood, L_particles, L_shp)]

        elif self.dnType == 'SVGD_t2_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
            ...
            lbd = 0
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
                                 tf.tensordot(exp_ij, 1. / (T**2) / (np.pi ** 2 / 6) * grad_log_prior, [[1], [0]]) + \
                                 lbd * ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles,
                                                                                                   [[1], [0]])))
                        for grad_log_prior, grad_log_likelihood, particles, shp in zip(L_grad_log_prior, L_grad_log_likelihood, L_particles, L_shp)]


        elif self.dnType == 'SVGD_k_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
            ...
            lbd = 0
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
                                 tf.tensordot(exp_ij, coeff * tf.tensordot(exp_ij, grad_log_prior, [[1], [0]]), [[1], [0]]) + \
                                 lbd * ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles,
                                                                                                   [[1], [0]])))
                        for grad_log_prior, grad_log_likelihood, particles, shp in zip(L_grad_log_prior, L_grad_log_likelihood, L_particles, L_shp)]

        elif self.dnType == 'SVGD_kt_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
            ...
            lbd = 0
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
                                 tf.tensordot(exp_ij, 1. / T * coeff * tf.tensordot(exp_ij, grad_log_prior, [[1], [0]]), [[1], [0]]) + \
                                 lbd * ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles,
                                                                                                   [[1], [0]])))
                        for grad_log_prior, grad_log_likelihood, particles, shp in zip(L_grad_log_prior, L_grad_log_likelihood, L_particles, L_shp)]
            ...
            # Diffusion 1/t
        elif self.dnType == 'SVGD_0_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_1_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_t2_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_k_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_kt_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
            ...
            # Diffusion e
        elif self.dnType == 'SVGD_0_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_1_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_t2_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_k_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_kt_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
            ...
            # Diffusion 1
        elif self.dnType == 'SVGD_0_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
                coeff = 1. / M_fl
            lbd = self.pm.lambda_c - (self.pm.lambda_c / (T ** 0.5))
            # lbd = self.pm.lambda_c
            # L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
            #                      tf.tensordot(exp_ij, 1. / (T**2) / (np.pi ** 2 / 6) * grad_log_prior, [[1], [0]]) + \
            #                      lbd * ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles,
            #                                                                                        [[1], [0]])))
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
                                 lbd * ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles,
                                                                                                   [[1], [0]])))
                        for grad_log_prior, grad_log_likelihood, particles, shp in
                        zip(L_grad_log_prior, L_grad_log_likelihood, L_particles, L_shp)]
        elif self.dnType == 'SVGD_1_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_t2_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_t2_logp_i_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
                coeff = 1. / M_fl
            # lbd = 1 - (self.pm.lambda_c / (T ** 0.5))
            lbd = self.pm.c1 - self.pm.c2 * T ** (1 / self.pm.c3)
            L_vecfld = [coeff * (tf.tensordot(exp_ij, grad_log_likelihood, [[1], [0]]) + \
                                 tf.tensordot(exp_ij, 1. / (T ** 2) / (np.pi ** 2 / 6) * grad_log_prior, [[1], [0]]) + \
                                 lbd * ibandw * (tf.reshape(exp_i, shp) * particles - tf.tensordot(exp_ij, particles,
                                                                                                   [[1], [0]])))
        ...
        elif self.dnType == 'SVGD_k_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        elif self.dnType == 'SVGD_kt_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl) / M_fl
            else:
        ...
        #     else: coeff = 1./M_fl
        #     L_vecfld = [coeff * ( tf.tensordot(exp_ij, grad_log_prior, [[1],[0]]) + ibandw * \
        #             (tf.reshape(exp_i, shp)*particles - tf.tensordot(exp_ij, particles, [[1],[0]])) )\
        #             for grad_log_prior, particles, shp in zip(L_grad_log_prior, L_particles, L_shp)]
        ...
        # elif self.dnType == 'online_1':
        #     L_vecfld = L_grad_log_likelihood
        #
        # elif self.dnType == 'online_2':
        #     x = tf.constant([1], dtype=tf.float64)
        #     coeff_tf = tf.divide(x,T)
        #     # coeff = 1/50
        #     # coeff_tf = tf.constant([coeff], dtype=tf.float64)
        #     L_vecfld = [grad_log_likelihood + coeff_tf * grad_log_prior \
        #                 for grad_log_likelihood, grad_log_prior in zip(L_grad_log_likelihood, L_grad_log_prior)]
        #     # L_vecfld = [grad_log_likelihood + coeff *grad_log_prior \
        #     #             for grad_log_likelihood, grad_log_prior in zip(L_grad_log_likelihood, L_grad_log_prior)]
        #
        #
        ...
        #     L_shp = [tf.concat([[M], tf.ones([rk-1], intype)], 0) for rk in L_rk]
        #     nor_exp_ij = exp_ij / exp_i
        #     L_vecfld = [grad_log_prior + ibandw * \
        ...
        # lbd = 0
        elif self.dnType == 'Blob_0_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            lbd = 0

            L_vecfld = [grad_log_likelihood +  ibandw * lbd * \
                        (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                         + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                        for grad_log_likelihood, grad_log_prior, particles, shp in zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]

        # elif self.dnType == 'Blob_1_logp_0_diffusion':
        #     L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
        #     nor_exp_ij = exp_ij / exp_i
        #     lbd = 0
        #
        #     L_vecfld = [grad_log_likelihood + grad_log_prior +  ibandw * lbd * \
        #                 (-particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), -particles, [[0], [0]]) \
        #                  + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
        #                 for grad_log_likelihood, grad_log_prior, particles, shp in zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]
###################
            ...
            L_vecfld = [grad_log_likelihood + 1. / T ** 2 * grad_log_prior +  ibandw * lbd * \
                        (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                         + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                        for grad_log_likelihood, grad_log_prior, particles, shp in zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]
            ################
        elif self.dnType == 'Blob_t2_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
            else:
                coeff = 1.
            lbd = 0

            L_vecfld = [grad_log_likelihood + 1. / (T**2) / (np.pi ** 2 / 6) * grad_log_prior + coeff * ibandw * lbd * \
                        (-particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), -particles, [[0], [0]]) \
                         + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * -particles) \
                        for grad_log_likelihood, grad_log_prior, particles, shp in zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]

        elif self.dnType == 'Blob_k_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
            ...
            L_vecfld = [grad_log_likelihood + tf.tensordot(exp_ij, grad_log_prior, [[1], [0]])  +   ibandw * lbd * \
                        (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                         + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                        for grad_log_likelihood, grad_log_prior, particles, shp in zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]

        elif self.dnType == 'Blob_kt_logp_0_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
            lbd = 0

            L_vecfld = [grad_log_likelihood + 1. / T  * tf.tensordot(exp_ij, grad_log_prior, [[1], [0]]) +   ibandw * lbd * \
                        (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                         + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                        for grad_log_likelihood, grad_log_prior, particles, shp in zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]

        # lbd = 1/t
        elif self.dnType == 'Blob_0_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            lbd = 1. / T
        ...
        elif self.dnType == 'Blob_1_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
            ...
            L_vecfld = [grad_log_likelihood + grad_log_prior +   ibandw * lbd * \
                        (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                         + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                        for grad_log_likelihood, grad_log_prior, particles, shp in
                        zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]

        elif self.dnType == 'Blob_t2_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
            ...
            L_vecfld = [grad_log_likelihood + 1. / (T**2) / (np.pi ** 2 / 6) * grad_log_prior +   ibandw * lbd * \
                        (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                         + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                        for grad_log_likelihood, grad_log_prior, particles, shp in
                        zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]

        elif self.dnType == 'Blob_k_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        elif self.dnType == 'Blob_kt_logp_t_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
            ...
            L_vecfld = [
                grad_log_likelihood + 1. / T * tf.tensordot(exp_ij, grad_log_prior, [[1], [0]]) +   ibandw * lbd * \
                (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                 + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                for grad_log_likelihood, grad_log_prior, particles, shp in
                zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]
        ...
        # lbd = e
        elif self.dnType == 'Blob_0_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            lbd = 1 - tf.exp(-T / 1000.)
        ...
        elif self.dnType == 'Blob_1_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        elif self.dnType == 'Blob_t2_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        elif self.dnType == 'Blob_k_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        elif self.dnType == 'Blob_kt_logp_e_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        # lbd = 1
        elif self.dnType == 'Blob_0_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            lbd = 1
        ...
        elif self.dnType == 'Blob_1_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        elif self.dnType == 'Blob_t2_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        elif self.dnType == 'Blob_k_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
        ...
        elif self.dnType == 'Blob_kt_logp_1_diffusion':
            L_shp = [tf.concat([[M], tf.ones([rk - 1], intype)], 0) for rk in L_rk]
            nor_exp_ij = exp_ij / exp_i
            if self.pm.dnNormalize:
                coeff = tf.pow(ibandw / (2. * np.pi), .5 * D_fl)
            ...
            L_vecfld = [
                grad_log_likelihood +  ibandw * lbd * \
                (particles - tf.tensordot((nor_exp_ij + tf.transpose(nor_exp_ij)), particles, [[0], [0]]) \
                 + tf.reshape(tf.reduce_sum(nor_exp_ij, axis=1), shp) * particles) \
                for grad_log_likelihood, grad_log_prior, particles, shp in
                zip(L_grad_log_likelihood, L_grad_log_prior, L_particles, L_shp)]
